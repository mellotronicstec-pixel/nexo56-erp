...
**apenas um token opaco** — nunca CPF, telefone ou endereço
([ADR-070](docs/adr/ADR-070-certificado-e-snapshot-com-token-opaco.md)).

**O certificado tem PDF real.** Um `application/pdf` de verdade, em A4,
multipágina, gerado no servidor a partir do mesmo snapshot — sem Chromium, sem
serviço separado, sem binário externo, e por isso sem exigir VPS. Baixar de novo
entrega o mesmo arquivo; se a política mudar amanhã, o documento continua
dizendo o que foi prometido hoje
([ADR-072](docs/adr/ADR-072-pdf-do-certificado-e-programatico.md)).
...
| Interface        | a recusa é dita, não escondida; a seção de garantias da OS não menciona WhatsApp, SMS, nota fiscal nem compra de peça (varredura do texto renderizado)     |

### Cobertura do certificado em PDF (Prompt 13.1)

| Área             | O que é testado                                                                                                                              |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| PDF real         | bytes começam com `%PDF-`, terminam em `%%EOF`, o parser abre, conta páginas e lê os metadados; **não** é HTML renomeado                     |
| Conteúdo         | título, referência, cliente, tipo, vigência, cobertura e paginação extraídos do fluxo de conteúdo, não de um snapshot de imagem              |
| Português        | `João Gonçalves`, `Márcia Araújo`, `Assistência Técnica São José`, `Condições de utilização`, maiúsculas acentuadas e cedilha — sem mojibake |
| Multipágina      | termos longos geram 2+ páginas; **as 16 cláusulas** estão presentes, não só as das bordas; rodapé numera todas com o total correto           |
| Quebra           | palavra de 300 caracteres sem espaço é fracionada e o conteúdo continua íntegro                                                              |
| Quatro tipos     | interna, fábrica, peça e estendida, com e sem Ordem de Serviço                                                                               |
| **Snapshot**     | alterar a política **e** o cadastro do cliente depois da emissão: o PDF continua idêntico, byte a byte                                       |
| Idempotência     | dez gerações sequenciais → um artefato; a linha do tempo registra a geração uma vez                                                          |
| **Concorrência** | **cinco gerações simultâneas em MariaDB real → uma chave, um arquivo novo no disco, cinco respostas válidas**                                |
| Recuperação      | arquivo apagado ou corrompido é detectado pelo checksum e regerado do snapshot histórico                                                     |
...
| PDF do certificado sem as fontes da marca        | Sora e Inter chegam pelo `next/font/google`, que as baixa no build para o navegador; não há arquivo no repositório para embutir, e buscar fonte pela rede durante a geração é proibido. O documento usa as fontes padrão do PDF, que cobrem todo o português | licenciar e versionar os arquivos de fonte                                                  |
| PDF do certificado sem logotipo                  | Os ativos oficiais do Nexo56 ainda não foram entregues, e a marca não é reconstruída com fonte. O documento identifica a empresa que concedeu a garantia                                                                                                     | receber os ativos oficiais                                                                  |
