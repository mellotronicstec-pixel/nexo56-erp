...
Todas as capacidades desta versão são de nível **tenant**: o catálogo atual
contém apenas administração estrutural. As permissões de operação diária
(ordens de serviço, estoque, caixa) chegam com os módulos de negócio e serão,
essas sim, tipicamente de **unidade** — o motor de escopo já está pronto e
testado para recebê-las.
