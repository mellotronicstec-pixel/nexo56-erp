...
| Telemetria de geração de IA                    | `ai_requests`                                                               | Interno                                   | **Sem conteúdo textual persistido.** Nenhuma coluna guarda prompt, texto de entrada ou saída — só contagem de caracteres, task, status, erro e tempo                                                                                                  |
| Busca de peças                                 | `part_search_sessions/candidates/evidence/offers/provider_calls/selections` | Interno                                   | **Sem PII.** `query_term` é técnico (peça/código), validado a 200 caracteres; contexto do equipamento carrega só tipo/marca/modelo — nunca relato do cliente, nunca CPF/CNPJ/telefone/endereço. Preço/oferta externos são temporais, nunca reescritos |
...
20. **`ai_requests` não guarda prompt, texto de entrada nem saída, por
    construção** (Prompt 20, itens 20 a 23). Não é política a lembrar — a
    tabela não tem coluna de texto, e a assinatura de
    `insertAiRequest`/`completeAiRequest` não aceita parâmetro de texto
    nenhum. Sem hash de conteúdo: um hash de texto curto é adivinhável e
    viraria fingerprint do próprio dado sensível que a tabela existe para
    não guardar. O texto que o usuário manda para a Nexo56 AI processar
    (relato do cliente, observação técnica) segue classificado como sempre
    foi na origem (`service_orders.internal_notes`, `quotes.customer_notes`)
    — a IA não muda a sensibilidade do dado, só o processa em trânsito, sem
    persistir cópia própria.
